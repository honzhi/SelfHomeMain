        ...
        if (Util.CheckIsHome(event.getTo().getWorld().getName().replace(Variable.world_prefix, ""))) {
            ...
            if (Variable.bungee) {
                if (MySQL.CheckIsAHome(name)) {
                    ...
                    if (!Util.Check(event.getPlayer(), event.getTo().getWorld().getName().replace(Variable.world_prefix, ""))
                        && !event.getPlayer().hasPermission("SelfHome.forcetp")
                        && !HomeAPI.getHome(event.getTo().getWorld().getName()).isAllowStranger()) {
                        p.sendMessage(Variable.Lang_YML.getString("HeadLineTtitle"));
                    ...
                    if (!event.getTo()
                        ...
                        if (!event.getPlayer().getName().equalsIgnoreCase(event.getTo().getWorld().getName().replace(Variable.world_prefix, ""))
                            && event.getPlayer().hasPermission("SelfHome.Popularity")) {
                            if (event.getPlayer().isOp()) {
                                ...
                                for (int i = 0; i < list.size(); i++) {
                                    if (list.get(i).equalsIgnoreCase(event.getTo().getWorld().getName().replace(Variable.world_prefix, ""))) {
                                        has_vote = true;
                                        break;
                                    }
                                }
                                    ...
                                    String.valueOf(
                                        (
                                                Main.JavaPlugin.getConfig().getInt("WorldBoard")
                                                    + (Integer.valueOf(MySQL.getLevel(name)) - 1) * Main.JavaPlugin.getConfig().getInt("UpdateRadius")
                                            )
                                            / 2
                                    )
                                );
